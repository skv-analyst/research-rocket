...
def get_interviews(project_id: int = None):
    data = db.read_db(
        "SELECT DISTINCT id, content FROM interviews WHERE project = $id", {"id": project_id}
    )
    return data


class LlmProcessInterview:
    def __init__(self, project_id: int = None, questions: dict = None):
